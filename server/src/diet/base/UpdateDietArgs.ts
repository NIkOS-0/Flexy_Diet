/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DietWhereUniqueInput } from "./DietWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DietUpdateInput } from "./DietUpdateInput";

@ArgsType()
class UpdateDietArgs {
  @ApiProperty({
    required: true,
    type: () => DietWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DietWhereUniqueInput)
  @Field(() => DietWhereUniqueInput, { nullable: false })
  where!: DietWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DietUpdateInput,
  })
  @ValidateNested()
  @Type(() => DietUpdateInput)
  @Field(() => DietUpdateInput, { nullable: false })
  data!: DietUpdateInput;
}

export { UpdateDietArgs as UpdateDietArgs };
